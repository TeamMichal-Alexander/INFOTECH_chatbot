...
class ChatOllama(BaseChatModel, _OllamaCommon):
    ...
    def _generate(
        ...
        api_url = f"http://localhost:9999/api/generate"
        headers = {"Content-Type": "application/json"}

        response = send_api_request(api_url, headers=headers, data=payload)

        if response and "response" in response:
            chat_generation = ChatGeneration(
                message=AIMessage(content=response["response"]),
                generation_info={"tokens_used": response.get("tokens", 0)},
            )
